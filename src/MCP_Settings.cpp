...
void MCP_Settings::read_out_settings() {
    ...
    for(int i = 0; i < 64; i++) {
        ...
        std::cout << "Read out settings: out" << i << " enabled:" << out_settings[i].out_enabled
                  << " bistable:" << out_settings[i].out_bistable << " dis_by_alarm"
                  << out_settings[i].out_disabled_by_alarm << " name:" << out_settings[i].out_name << std::endl;
    }
}
...
void MCP_Settings::read_in_settings() {
    ...
    for(int i = 0; i < 64; i++) {
        ...
        std::cout << "Read in settings: in:" << i << " ou:" << unsigned(in_settings[i].related_output)
                  << " enabled:" << in_settings[i].in_enabled << " - name:" << in_settings[i].in_name << std::endl;
    }
}
