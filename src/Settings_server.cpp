...
void SettingsServer::analyze_packet(SERIALMCPFRAME frame) {
    ...
    if(frame.INSTRUCTIONS == 0x00 & frame.CONFIG == 0x01) {
        ...
        std::cout << "set_io_relation: " << unsigned(frame.IONUMBER) << " - "
                  << unsigned(mcp_settings->get_io_relation(frame.IONUMBER)) << std::endl;
    } else if(frame.INSTRUCTIONS == 0x00 & frame.CONFIG == 0x00) {
        std::cout << "get_io_relation: " << unsigned(frame.IONUMBER) << " - "
                  << unsigned(mcp_settings->get_io_relation(frame.IONUMBER)) << std::endl;
    }

    else if(frame.INSTRUCTIONS == 0x01 & frame.CONFIG == 0x01) {
        mcp_settings->set_in_status(frame.IONUMBER, frame.VALUE);
        std::cout << "set_in_status: " << unsigned(frame.IONUMBER) << " - "
                  << unsigned(mcp_settings->get_in_status(frame.IONUMBER)) << std::endl;
    } else if(frame.INSTRUCTIONS == 0x01 & frame.CONFIG == 0x00) {
        std::cout << "get_in_status: " << unsigned(frame.IONUMBER) << " - "
                  << unsigned(mcp_settings->get_in_status(frame.IONUMBER)) << std::endl;
    }

    else if(frame.INSTRUCTIONS == 0x02 & frame.CONFIG == 0x01) {
        mcp_settings->set_out_status(frame.IONUMBER, frame.VALUE);
        std::cout << "set_out_status: " << unsigned(frame.IONUMBER) << " - "
                  << unsigned(mcp_settings->get_out_status(frame.IONUMBER)) << std::endl;
    } else if(frame.INSTRUCTIONS == 0x02 & frame.CONFIG == 0x00) {
        std::cout << "get_out_status: " << unsigned(frame.IONUMBER) << " - "
                  << unsigned(mcp_settings->get_out_status(frame.IONUMBER)) << std::endl;
    }

    else if(frame.INSTRUCTIONS == 0x03 & frame.CONFIG == 0x01) {
        mcp_settings->set_out_bistable(frame.IONUMBER, frame.VALUE);
        std::cout << "set_out_bistable: " << unsigned(frame.IONUMBER) << " - "
                  << unsigned(mcp_settings->get_out_bistable(frame.IONUMBER)) << std::endl;
    } else if(frame.INSTRUCTIONS == 0x03 & frame.CONFIG == 0x00) {
        std::cout << "get_out_bistable: " << unsigned(frame.IONUMBER) << " - "
                  << unsigned(mcp_settings->get_out_bistable(frame.IONUMBER)) << std::endl;
    }

    else if(frame.INSTRUCTIONS == 0x04 & frame.CONFIG == 0x01) {
        mcp_settings->set_in_name(frame.IONUMBER, std::string(frame.NAME));
        std::cout << "set_in_name: " << unsigned(frame.IONUMBER) << " - " << mcp_settings->get_in_name(frame.IONUMBER)
                  << std::endl;
    } else if(frame.INSTRUCTIONS == 0x04 & frame.CONFIG == 0x00) {
        std::cout << "get_in_name: " << unsigned(frame.IONUMBER) << " - " << mcp_settings->get_in_name(frame.IONUMBER)
                  << std::endl;
    }

    else if(frame.INSTRUCTIONS == 0x05 & frame.CONFIG == 0x01) {
        mcp_settings->set_out_name(frame.IONUMBER, std::string(frame.NAME));
        std::cout << "set_in_name: " << unsigned(frame.IONUMBER) << " - " << mcp_settings->get_out_name(frame.IONUMBER)
                  << std::endl;
    } else if(frame.INSTRUCTIONS == 0x05 & frame.CONFIG == 0x00) {
        std::cout << "get_in_name: " << unsigned(frame.IONUMBER) << " - " << mcp_settings->get_out_name(frame.IONUMBER)
                  << std::endl;
    }

    else if(frame.INSTRUCTIONS == 0xff & frame.CONFIG == 0x01) {
        mcp_settings->save_settings();
        std::cout << "write_settings: " << std::endl;
    } else if(frame.INSTRUCTIONS == 0xff & frame.CONFIG == 0x00) {
